...
import { capabilityChapters } from "@/lib/content";
...
export function ScrollStory() {
  ...
  return (
    <section id="story" ref={containerRef} className="relative">
      <div className="sticky top-[var(--site-chrome-height,7.75rem)] flex h-[calc(100vh-var(--site-chrome-height,7.75rem))] items-center overflow-hidden">
        ...
        <div className="relative mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <p className="mt-4 max-w-2xl text-muted-foreground">
            Четыре этапа на одной линии — от постановки до релиза. Циклом или одним
            участком; scope и приёмка до макета.
          </p>

          <div className="mt-12 grid gap-8 lg:grid-cols-2 lg:gap-16">
            <div className="space-y-4">
              {capabilityChapters.map((chapter, index) => {
                const start = index / capabilityChapters.length;
                const end = (index + 1) / capabilityChapters.length;
                return (
                  <ChapterIndicator
                    key={chapter.id}
                    chapter={chapter}
      ...
      <div style={{ height: `${capabilityChapters.length * 60}vh` }} />
    </section>
  );
}

type Chapter = (typeof capabilityChapters)[number];
      ...
      >
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {chapter.tags.map((tag) => (
            <span
              key={tag}
              className="rounded-full border border-white/10 bg-white/5 px-3 py-1 font-mono text-[10px] uppercase tracking-wider text-muted-foreground"
            >
              {tag}
            </span>
          ))}
